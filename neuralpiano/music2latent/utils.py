...
def download_models():
    ...
    if not os.path.exists(os.path.join(models_dir, "music2latent_maestro_loss_16.871_iters_45500.pt")):
        ...
        for repo_id, filename, repo_type in files:
            print(f"Downloading {filename} from {repo_id} ...")
            # download to Hugging Face cache and get the real path returned
            cached_path = hf_hub_download(repo_id=repo_id, repo_type=repo_type, filename=filename, local_dir=models_dir)
